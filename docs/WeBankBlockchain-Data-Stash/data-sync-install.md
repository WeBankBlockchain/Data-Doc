...
请在要同步数据的节点上执行以下步骤，同步前新节点保持在未启动状态。
...
上述脚本会自动拉取对应系统的data-sync包，并自动读取导出群组的配置，同步数据到对应的数据存储中，节点存储模式包括rocksdb/mysql/scalable三种。其中scalable模式，只同步状态数据，区块数据将从数据仓库获取。
